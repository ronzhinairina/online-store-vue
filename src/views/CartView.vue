...
<template>
  <div>
    ...
    <div v-if="shop.cart.length > 0" class="flex flex-col lg:flex-row lg:gap-6">
      <div class="divide-y divide-gray-300 lg:w-2/3">
        <div v-for="item in shop.cart" :key="item.id" class="py-6 flex gap-4">
          <img class=" cart-product w-32 bg-beige rounded-md" :src="item.image" />
      ...
      <div class="bg-gray-100 rounded-md p-6 lg:w-1/3 flex flex-col gap-6">
        ...
        <div class="divide-y divide-gray-300">
          <div class="py-3 flex justify-between font-bold">
            <div>Order total</div>
            <div>${{ shop.cart.reduce((sum, curr) => sum += curr.price * curr.quantity, 0) }}</div>
          </div>
        </div>
